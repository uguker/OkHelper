...
public class ProgressCircelView extends View {
    ...
    private void drawLine(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //canvas.drawArc(getReact(), routeDegree + i * (360 / count), getDrawDegree(), false, paint);
            float x1   = (float) (width / 2   +   width / 2   *   Math.cos((routeDegree + i * (360 / count))   *   Math.PI   /180));
            float yi   = (float) (height / 2   +   width / 2   *   Math.sin((routeDegree + i * (360 / count))   *   Math.PI  /180));
            paint.setColor(Color.BLACK);
            canvas.drawPoint(x1, yi, paint);
        }
    ...
    private float getPaintWidth() {
        //画笔的初始大小为30，变化范围为15
        return (float) (getRouteNumber() * 3 + 9);
    }
